...
pframe_alloc(mmobj_t *o, uint32_t pagenum)
{
	...
	dbg(DBG_FORK, "mmobj %p nres from %d to %d\n", o, o->mmo_nrespages,
	    o->mmo_nrespages + 1);
        o->mmo_ops->ref(o);
        o->mmo_nrespages++;
...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
	...
	/* check if the page is resident in the memory and return it if so. */
        *result = pframe_get_resident(o, pagenum);
	if (*result)
		return 0;

	/* At this point, the page is not in the memory. So,
	 * 1. alloc a new page and fill it.
	 * 2. check if we need to run pageoutd and if so, wake it up.
	 */
...
{
	/* dbg(DBG_PFRAME, "pinning %p from %d to %d\n", pf, pf->pf_pincount, pf->pf_pincount + 1); */
	/* if not pinned, move to pinlist */
	if (pframe_is_pinned(pf)) {
		pf->pf_pincount++;
		return;
	}
...
{
	/* dbg(DBG_PFRAME, "unpinning %p from %d to %d\n", pf, pf->pf_pincount, pf->pf_pincount - 1); */
	...
	/* if pin count reaches 0, move it to allocated list */
	dbg(DBG_PFRAME, "npinned = %d to %d\n", npinned, (npinned - 1));
	list_remove(&pf->pf_link);
	npinned--;
	KASSERT(npinned >= 0);
...
/*
 * This function is only used by anon_put,
 * where o->mmo_ops->put will cause a chain.
 */
void
pframe_free_no_put(pframe_t *pf)
{
        KASSERT(!pframe_is_pinned(pf));
        KASSERT(!pframe_is_free(pf));
        KASSERT(!pframe_is_busy(pf));
