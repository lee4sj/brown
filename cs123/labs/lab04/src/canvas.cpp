 ...
 */
void Lab5Canvas::subdivideSquare(GridIndex topleft, GridIndex botright, GLint curDepth)
{
    // TL--TM--TR    +---> x
    // |   |   |     |
    // ML--MM--MR    V
    ...
    GridIndex TM = GridIndex((TR + TL) / 2);
    GridIndex BM = GridIndex((BR + BL) / 2);
    ...
    // set the worldpoints
    vML = Vector3((vTL.x + vBL.x)/2, (vTL.y + vBL.y)/2, (vTL.z + vBL.z)/2);
    vMR = Vector3((vTR.x + vBR.x)/2, (vTR.y + vBR.y)/2, (vTR.z + vBR.z)/2);
    vTM = Vector3((vTR.x + vTL.x)/2, (vTR.y + vTL.y)/2, (vTR.z + vTL.z)/2);
    vBM = Vector3((vBR.x + vBL.x)/2, (vBR.y + vBL.y)/2, (vBR.z + vBL.z)/2);
    vMM = Vector3((vTL.x + vTR.x + vBL.x + vBR.x)/4, (vTL.y + vTR.y + vBL.y + vBR.y)/4 + getPerturb(curDepth), (vTL.z + vTR.z + vBL.z + vBR.z)/4);
...
void Lab5Canvas::computeNormals()
{
    ...
    for (int row = 0; row < m_gridLength; row++)
    {
        for (int column = 0; column < m_gridLength; column++)
        {
            ...
            // @TODO: [Lab 5] Compute cross products for each neighbor    loadTexture();
...
void Lab5Canvas::paintGL()
{
    ...
    for (int row = 0; row < m_gridLength-1; row++) {
        ...
        for (int col = 0; col < m_gridLength; col++) {
            glTexCoord2f(col/(float)m_gridLength, row/(float)m_gridLength);
            glNormal3dv(m_normalMap[getIndex(row, col)].data);
            glVertex3dv(m_terrain[getIndex(row, col)].data);

            glTexCoord2f(col/(float)m_gridLength, (row+1)/(float)m_gridLength);
            glNormal3dv(m_normalMap[getIndex(row + 1, col)].data);
            glVertex3dv(m_terrain[getIndex(row + 1, col)].data);
        }
        glEnd();
    }
...
/**
  ...
  */
inline int Lab5Canvas::getIndex(const GridIndex &c)
{
 ...
 */
QList<WorldPoint*> Lab5Canvas::getSurroundingVertices(const GridIndex &coordinate)
{
    GridIndex coords[8];
