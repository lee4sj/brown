...
//    cout << " --- Finish Loading Resources ---" << endl;
    ...
    loadTexture(QString("../projects/textures/NormalMap.jpg"));
}
...
GLuint GLWidget::loadTexture(const QString &filename)
{
    ...
    QFile file(filename);
    if (!file.exists()) {
        cout << "texture map not loaded" << endl;
        return -1;
    }
    cout << "loading texture map" << endl;
...
void GLWidget::createShaderPrograms()
{
    ...
    m_shaderPrograms["bump"] = ResourceLoader::newShaderProgram(ctx, "../final/shaders/bump.vert",
                                                                "../final/shaders/bump.frag");
}
...
//    m_shaderPrograms["reflect"]->setUniformValue("CubeMap", GL_TEXTURE0);
    ...
    m_shaderPrograms["bump"]->setUniformValue("diffuseTexture", GL_TEXTURE0);
    glPushMatrix();
