...
sys_read(read_args_t *arg)
{
	...
	while (total_read < a.nbytes) {
		cur_nbytes = MIN(a.nbytes - total_read, PAGE_SIZE);

		if ((rd = do_read(a.fd, temp_buf, cur_nbytes)) < 0) {
			page_free(temp_buf);
			curthr->kt_errno = -rd;
			return -1;
		} else if (rd == 0)
		...
		if ((ret = copy_to_user((void *)((uintptr_t)a.buf + total_read),
					temp_buf, rd)) < 0) {
			page_free(temp_buf);
			curthr->kt_errno = -ret;
			return -1;
...
sys_write(write_args_t *arg)
{
	...
	total_written = 0;
	cur_nbytes = MIN(a.nbytes, PAGE_SIZE);

	while (total_written < a.nbytes) {
		if ((ret = copy_from_user(temp_buf,
					  (void *)((uintptr_t)a.buf +
						   total_written),
					  cur_nbytes)) < 0) {
			page_free(temp_buf);
			curthr->kt_errno = -ret;
		...
		if ((ret = do_write(a.fd, temp_buf, cur_nbytes)) < 0) {
			page_free(temp_buf);
			curthr->kt_errno = -ret;
			return -1;
		} else if (ret == 0) {
		...
		cur_nbytes = MIN(a.nbytes - total_written, PAGE_SIZE);
	}
...
sys_getdents(getdents_args_t *arg)
{
	...
	/* copy arguments to the kernel from the user */
	if (copy_from_user(&a, arg, sizeof(a)) < 0) {
		curthr->kt_errno = EFAULT;
		return -1;
	}
	...
	/* call do_getdent (a.count / sizeof(dirent_t)) times */
	dirent_t *d_ptr = a.dirp;
	while (total_read < a.count) {
		if ((rd = do_getdent(a.fd, &d)) < 0) {
			curthr->kt_errno = -rd;
			return -1;
		} else if (rd == 0)
			break;

		KASSERT(rd == sizeof(dirent_t));

		if ((ret = copy_to_user(d_ptr, (void *)&d, rd)) < 0) {
			curthr->kt_errno = -ret;
			return -1;
		}
