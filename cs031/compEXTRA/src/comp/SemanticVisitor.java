...
public class SemanticVisitor implements Visitor {
	...
	private HashMap<String, ArrayList<ID>> structList;	//collection to keep track of structs
	...
	public void handleNodeInstrDeclArray(NodeInstrDeclArray node) throws VisitorException {
		...
		ID temp3 = new ID(node.identifier(), IDType.STRUCT);
        if(idList.contains(temp) || idList.contains(temp2) || idList.contains(temp3))
	...
	public void handleNodeExprArrayVar(NodeExprArrayVar node) throws VisitorException {
        ...
        if(!idList.contains(new ID(node.variable(), IDType.ARRAY)))
            die("a variable is used without declaration or is a scalar or is a struct, but used as an array!");
...
/*
	public void handleNodeInstrDeclPointer(NodeInstrDeclPointer node) throws VisitorException{
		//visit child
		checkAndVisit(node.val());
	...
	public void handleNodeExprArrayVarPointer(NodeExprArrayVarPointer node) throws VisitorException{
	...
	public void handleNodeExprVarPointer(NodeExprVarPointer node) throws VisitorException{
		//***** scope analysis
        if(!idList.contains(new ID(node.variable(), IDType.SINGLE)) && !idList.contains(new ID(node.variable(), IDType.ARRAY)))
            die("a variable is used without declaration!");

        //***** type checking
        typeStack.push(node.semanticType());
	}

	public void handleNodeExprVarDeref(NodeExprVarDeref node) throws VisitorException{
		//***** scope analysis
        if(!idList.contains(new ID(node.variable(), IDType.SINGLE)))
            die("a variable is used without declaration or is an array or is a struct, but used as a scalar!");

        //***** type checking
        typeStack.push(node.semanticType());
	}

	public void handleNodeExprArrayVarDeref(NodeExprArrayVarDeref node) throws VisitorException{
        checkAndVisit(node.index());
	...
	public void handleNodeInstrAssignSinglePointer(NodeInstrAssignSinglePointer node) throws VisitorException{

		checkAndVisit(node.expression());

		//***** scope analysis
        if(!idList.contains(new ID(node.identifier(), IDType.SINGLE)) && !idList.contains(new ID(node.identifier(), IDType.ARRAY)));
            die("a variable is used without declaration or is a struct!");
	...
	public void handleNodeInstrAssignArrayPointer(NodeInstrAssignArrayPointer node) throws VisitorException{
		checkAndVisit(node.index());
        checkAndVisit(node.expression());
	...
	public void handleNodeInstrAssignStruct(NodeInstrAssignStruct node) throws VisitorException{

		checkAndVisit(node.expression());
		checkAndVisit(node.variableInStruct());

		//****** scope analysis:
		if(!idList.contains(new ID(node.structName(), IDType.STRUCT)))
			die(" a variable is used without declaration or is used as other structure variable name(not as struct)!");

		//****** type checking
		if(typeStack.pop() != SemanticType.INTEGER || typeStack.pop() != SemanticType.INTEGER)
			die(" The struct assignment to " + node.structName() + " has invalid semantic type as input!");


	}

	public void handleNodeExprStruct(NodeExprStruct node) throws VisitorException{

		checkAndVisit(node.variableInStruct());

		//***** scope analysis
		if(!idList.contains(new ID(node.variable(), IDType.STRUCT)))
			die(" a variable is used without declaration or is used as other structure variable name(not as struct)!");

		//***** type checking
		if(typeStack.pop() != SemanticType.INTEGER)
			die( " invalid semantic type for " + node.variable());
		typeStack.push(node.semanticType());
	}

	public void handleNodeInstrDeclConsStruct(NodeInstrDeclConsStruct node) throws VisitorException{

		checkAndVisit(node.variableList());

		//***** scope analysis
		if(structList.containsKey(node.identifier()))
			die(" the struct is already defined!!");

		structList.put(node.identifier(), value)

	}

	public void handleNodeInstrDeclStruct(NodeInstrDeclStruct node) throws VisitorException{

	}

	public void handleNodeListDecl(NodeListDecl node) throws VisitorException{

	}

	public void handleNodeListDeclCons(NodeListDeclCons node) throws VisitorException{

	}

	public void handleNodeListDeclNil(NodeListDeclNil node) throws VisitorException{

	}

	public void handleNodeInstrInputStruct(NodeInstrInputStruct node ) throws VisitorException{
