...
void *rip_loop(void *arg) {
   rip_args *r = (struct rip_args *)arg;
   list_t *intrf_param = r->interface_lst;
   rip_init(intrf_param);

   // Signal ready
   pthread_mutex_lock(r->ready_mutex);
   *(r->ready_flag) = 1;
   pthread_mutex_unlock(r->ready_mutex);
...
interface *get_interface(struct in_addr *virt_ip){
   ...
   //inet_ntop(AF_INET, virt_ip, ip, INET_ADDRSTRLEN);
  //printf("get_interface called: %s\n", ip);
...
int send_request(interface* intrf){
  ...
  memcpy(packet, message, 2*sizeof(uint16_t));
...
int update_rip_table(struct entries *ent_list, uint16_t num_ent, struct in_addr *src){
  ...
  src_intrf->time_stamp = cur_time.tv_sec;
  ...
  for(int i = 0; i < num_ent; i++){
    ...
    // if cur_rip is local destination
    else if(cur_rip->cost == 0){
      //do nothing.
    }


    //since the src entry is already updated, leave it.    
    else if (cur_rip->network_addr != src->s_addr){
...
int handle_rip_packet(char *packet, size_t size, struct in_addr *src){
  ...
  if( command == 1){
    ...
    src_rip->next_hop->time_stamp = cur_time.tv_sec;
...
int check_expired(){
  ...
  for(list_link_t *link = interface_list->l_next;
    ...
    if((cur_sec - cur_intrf->time_stamp) >= EXPIREDTIME){
      trigger_updates = 1;
      interface_down(cur_intrf->id);
    }
  }
