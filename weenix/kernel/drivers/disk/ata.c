...
ata_read(blockdev_t *bdev, char *data, blocknum_t blocknum, unsigned int count)
{
	...
	while (!error && index < count) {
		error = ata_do_operation(adisk, data, blocknum + index, 0);
		index++;
	}
 ...
 */
static int
ata_write(blockdev_t *bdev, const char *data, blocknum_t blocknum,
	  unsigned int count)
{
	ata_disk_t *adisk = bd_to_ata(bdev);
	int error = 0;
	...
	while (!error && index < count) {
		error = ata_do_operation(adisk, (char *)data,
					 blocknum + index, 1);
		index++;
	}
...
ata_do_operation(ata_disk_t *adisk, char *data, blocknum_t blocknum, int write)
{
	...
	/* initialize DMA for the operation */
	dma_load(adisk->ata_channel, data, BLOCK_SIZE, write);

	/* tell the disk about the number of sectors to read/write
	   and the starting number. */
	ata_outb_reg(adisk->ata_channel, ATA_REG_SECCOUNT0,
		     adisk->ata_sectors_per_block);

	uint32_t sector = blocknum * adisk->ata_sectors_per_block;
	ata_outb_reg(adisk->ata_channel, ATA_REG_LBA0, (uint8_t)sector);
	ata_outb_reg(adisk->ata_channel, ATA_REG_LBA1, (uint8_t)(sector >> 8));
	ata_outb_reg(adisk->ata_channel, ATA_REG_LBA2, (uint8_t)(sector >> 16));

	/* tell the disk the type of the operation */
	ata_outb_reg(adisk->ata_channel, ATA_REG_COMMAND,
