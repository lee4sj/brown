...
int v_write_all(int s, const void *buf, size_t bytes_requested){
  ...
  while (bytes_written < bytes_requested){
    ...
    if (ret < 0){
      fprintf(stderr, "warning: v_write() error!\n");
      return ret;
    }
    if (ret == 0){
...
void interfaces_cmd(const char *line, list_t *interfaces_lst, pthread_mutex_t *m){
  (void)line;
   print_interface_info(interfaces_lst, m);
  return;
}
...
void sockets_cmd(const char *line){
  (void)line;
   ...
   pthread_mutex_t *cm = get_connections_mutex();
   int sockfd;
   connection *conn;
   ...
   htable_iterate_begin(connections, sockfd, conn, connection) {
      ...
      inet_ntop(AF_INET, &(conn->local_addr), lip, INET_ADDRSTRLEN);
      printf("    local_addr = %s\n", lip);
      printf("    local_port = %u\n", conn->local_port);
      char rip[INET_ADDRSTRLEN];
      inet_ntop(AF_INET, &(conn->remote_addr), rip, INET_ADDRSTRLEN);
      printf("    remote_addr = %s\n", rip);
      printf("    remote_port = %u\n", conn->remote_port);
      printf("    local_seq = %u\n", conn->local_seq); 
      printf("    remote_seq = %u\n", conn->remote_seq);
      printf("    last_read = %u\n", conn->last_read);
      printf("    next_expected = %u\n", conn->next_expected);
      printf("    last_ack = %u\n", conn->last_ack);
      printf("    last_written = %u\n", conn->last_written);
      printf("    last_sent = %u\n", conn->last_sent);
	...
	printf("\t RTO = %u\n", conn->send_root->next->timestamp.tv_sec);
...
void down_cmd(const char *line, list_t *if_lst, pthread_mutex_t *m){
  unsigned interface;
  int ret;
  ...
  handle_updown_command(interface, 0, if_lst, m);
}

void up_cmd(const char *line, list_t *if_lst, pthread_mutex_t *m){
  unsigned interface;
  int ret;
  ...
  handle_updown_command(interface, 1, if_lst, m);
}
...
void accept_cmd(const char *line){
  ...
  assert(ret == 0);
  printf("pthread create in accept\n"); // TODO
...
void sendfile_cmd(const char *line){
  ...
  ret = pthread_attr_setdetachstate(&thr_attr, PTHREAD_CREATE_DETACHED);
  assert(ret == 0);
  printf("pthread create in sendfile\n"); // TODO
...
void recvfile_cmd(const char *line){
  ...
  ret = pthread_attr_setdetachstate(&thr_attr, PTHREAD_CREATE_DETACHED);
  assert(ret == 0);
  printf("pthread create in recvfile\n"); // TODO
...
struct {
  const char *command;
  void (*handler)(const char *, list_t *if_l, pthread_mutex_t *m);
} cmd_table[] = {
  {"help", help_cmd},
...
void driver(list_t *interface_lst, pthread_mutex_t *if_mutex){
  char line[LINE_MAX];
  char cmd[LINE_MAX];
  char *fgets_ret;
  int ret;
  ...
  while (1){
    ...
    for (i=0; i < sizeof(cmd_table) / sizeof(cmd_table[0]); i++){
      if (!strcmp(cmd, cmd_table[i].command)){
        cmd_table[i].handler(line, interface_lst, if_mutex);
        break;
      }
    }
...
int main(int argc, char **argv){
  ...
  */
  ...
  list_t *interface_lst = get_interface_list();
  pthread_mutex_t *m = get_interface_mutex();
  //driver(interface_lst, &if_mutex);
  driver(interface_lst, m);
